...
const CMETareas = ({ navigation }) => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('http://apidextek.fragomx.com/api/auth/usuarios2').then(response => response.json()),
      fetch('http://apidextek.fragomx.com/api/auth/clientes').then(response => response.json()),
      fetch('http://apidextek.fragomx.com/api/auth/tareas').then(response => response.json()),
    ])
  ...
  const deleteTask = (taskId) => {
    fetch(`http://apidextek.fragomx.com/api/auth/eliminartarea/${taskId}`, {
      method: 'DELETE'
    })
