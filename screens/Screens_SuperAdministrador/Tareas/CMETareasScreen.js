...
<<<<<<< HEAD
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('http://192.168.1.10:3000/api/auth/tareas').then(response => response.json()),
    ])
  ...
  const deleteTask = (taskId) => {
    fetch(`http://192.168.1.10:3000/api/auth/eliminartarea/${taskId}`, {
      method: 'DELETE'
    })
        ...
        <View style={styles.container}>
          {(searchQuery.length > 0 ? filteredTasks : tasks).map(task => (
            <View key={task.id} style={styles.projectCard}>
              ...
              <Text style={styles.taskText}>Cliente: {task.client_name}</Text>
