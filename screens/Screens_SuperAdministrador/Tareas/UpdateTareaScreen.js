...
const UpdateTareaScreen = ({ route, navigation }) => {
  ...
  const fetchStatus = () => {
    fetch('http://192.168.100.115:3000/api/auth/status')
  ...
  const fetchProjects = () => {
    fetch('http://192.168.100.115:3000/api/auth/proyectos')
      .then(response => response.json())
  ...
  const updateTask = () => {
    ...
    fetch(`http://192.168.100.115:3000/api/auth/actualizartarea/${taskId}`, {
        method: 'PUT',
        headers: {
